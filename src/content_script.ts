...
function findMicButton() {
    ...
    if (_mic_button_element === null) {
        document.querySelectorAll('[data-tooltip]').forEach((element: HTMLElement) => {
            if (element.dataset.tooltip && (element.dataset.tooltip.toLowerCase().includes('⌘ + d') || muteButtonTooltip.test(element.dataset.tooltip))) {
                _mic_button_element = element;
                meetUI = 'old';
            }
        })
    ...
    if (_mic_button_element !== null && micButton !== _mic_button_element) {
        micButton = _mic_button_element;
        listenForMicButtonClick();
    }
    // Set the mic at the start to muted
    if (initialised === false && micButton && micButton.dataset.isMuted === 'false') {
        initialised = true;
        toggleMicButton();
    }
}
...
setInterval(() => {
    if (meetUI === 'old') {
        const topbuttonBar = document.querySelector('[data-allow-clicks]')
        if (!topbuttonBar) {
            return;
        }
        createPttButtonOldUI();
    } else if (meetUI === 'new') {
        createPttButtonNewUI();
    }
}, 250);

function createPttButtonOldUI() {
    const topbuttonBar = document.querySelector('[data-allow-clicks]')
    const buttons = topbuttonBar.parentElement.lastElementChild;
    ...
    if (!buttons.classList.contains('ptt_initialised') && !chatOpen) {
        ...
        pttButtonContainer.addEventListener('click', togglePTTApp);
...
function createPttButtonNewUI() {
    const buttons = micButton.parentElement.parentElement.parentElement.parentElement.parentElement;
    if (!buttons.classList.contains('ptt_initialised')) {
        ...
        const pttButtonContainer: any = document.createElement('div');
        pttButtonContainer.classList = buttons.children[1].classList;
        pttButtonContainer.id = '__ptt-button-container-new';
        pttButtonContainer.addEventListener('click', togglePTTApp);
        buttons.prepend(pttButtonContainer);

        const pttButtonToggleButton: any = document.createElement('span');
        pttButtonToggleButton.id = '__ptt-button-new';
        pttButtonToggleButton.classList = micButton.classList;
        pttButtonContainer.prepend(pttButtonToggleButton)
        ...
        pttButtonToggleButtonSVG.style.height = '24px'
        pttButtonToggleButtonSVG.setAttribute('viewBox', '0 0 24 24')
        document.querySelector('#__ptt-button-new').appendChild(pttButtonToggleButtonSVG)

        const pttButtonBackground = document.createElement('div');
        pttButtonBackground.id = '__ptt-button-background-new';
        pttButtonContainer.appendChild(pttButtonBackground)
